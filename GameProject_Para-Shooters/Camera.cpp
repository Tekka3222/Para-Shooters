...
void Camera::ActivateEnemies() {
	auto itr = EntityList[TYPE_ENEMY].begin();
	while (itr != EntityList[TYPE_ENEMY].end()) {
		if ((*itr)->GetHealth() <= 0) {
			(*itr)->SetState(State::Dead);
			(*itr)->GetGun()->SetState(State::Dead);
			for (int i = 0; i < NUM_OF_PARTICLES; i++) {
				(*itr)->GetParticle(i)->SetState(State::Dead);
			}
			itr = EntityList[TYPE_ENEMY].erase(itr);
			if (itr == EntityList[TYPE_ENEMY].end()) {
				return;
			}
		}
		Dir_Vector pCam = GetPosition();
		Dir_Vector eAbsPos = (*itr)->GetAbsPPos();
		Dir_Vector relativePos = eAbsPos - pCam;
		if (-Game::getWidth() * ENEMY_ACTIVE_RANGE < relativePos.x + (*itr)->getSize().x && relativePos.x < Game::getWidth() * (1 + ENEMY_ACTIVE_RANGE) && -Game::getHeight() * ENEMY_ACTIVE_RANGE < relativePos.y + (*itr)->getSize().y && relativePos.y < Game::getHeight() * (1 + ENEMY_ACTIVE_RANGE)) {
			(*itr)->setActive();
		}
		else {
			(*itr)->setNoActive();
...
void Camera::SetRandomMap() {
	...
	mt19937 mt;
	mt.seed(rnd());
	//Bound->0-SHEIGHT - 1(19)までの足場の高さを表す
	uniform_int_distribution<> rand(0, PROB_MAX);
	Bound[0] = DEFAULTHEIGHT;
	Bound[1] = DEFAULTHEIGHT;
	for (int i = 2; i < SWIDTH / ONESCAF + 1; i++) {
		int rvalue = rand(mt);
		//次の足場設置位置の始点に対する割合
		double placementRandomValue = (double)rand(mt) / 100.0;
		int before = Bound[i - 1];
		//以前の足場の近くに生成される場合(50％)
		if (rvalue <= PROB_NEAR) {
			int start = max(before - 5, 0), end = min(before + 5, SHEIGHT - 1);
			Bound[i] = start + static_cast<int>(placementRandomValue * ((double)end - start));
		}
		//以前の足場から少しはなれた場所生成される場合(30％)
		else if (rvalue <= PROB_NEAR + PROB_MID) {
			int start = max(before - 10, 0), end = min(before + 10, SHEIGHT - 1), nostart = max(before - 5, 0), noend = min(before + 5, SHEIGHT - 1);
			int length = (end - start) - (noend - nostart);
			Bound[i] = start + static_cast<int>(placementRandomValue * length);
			if (nostart <= Bound[i] && Bound[i] <= noend) {
				Bound[i] += (noend - nostart);
			}
		}
		//以前の足場からかなりはなれた場所生成される場合(10％)
		else if (rvalue <= PROB_NEAR + PROB_MID + PROB_DISTANT){
			int start = 0, end = SHEIGHT - 1, nostart = max(before - 10, 0), noend = min(before + 10, SHEIGHT - 1);
			int length = (end - start) - (noend - nostart);
			Bound[i] = start + static_cast<int>(placementRandomValue * length);
			if (nostart <= Bound[i] && Bound[i] <= noend) {
				Bound[i] += (noend - nostart);
		...
		//以前と同じ場所に生成される確率(10％(50％の確立で以前の足場の近くに生成されるが、その場合にも5％の確率で同じ場所に生成される))
		else {
			Bound[i] = before;
		}
	}
...
void Camera::CreateEnemies() {
	...
	mt19937 mt;
	mt.seed(rnd());
	//敵の出現し始める座標(足場3つめ)から、実際に出現させる位置までのマップ全体のx距離に対する割合をランダムに生成する。(0-100％)
	uniform_int_distribution<> randProbability(0, PROB_MAX);
	//敵の出現開始座標(足場3つめ)
	int beginToSpawn = ONESCAF * 3;
	//敵が出現する範囲
	int spawnLength = (SWIDTH - 1) - beginToSpawn + 1;
	map<int, bool> EPlace;
	//各マスに対する出現確率
	map<int, double> spawnPosibility;
	//1マス当たりの確率をspawnLengthを使って均等になるように初期設定し、その確率を求める
	for (int i = beginToSpawn; i < SWIDTH; i++) {
		spawnPosibility[i] = (PROB_MAX / (double)spawnLength);
	}
	//ステージにて出現する敵の数(最大MAX_NUM_OF_ENEMY)
	int numOfEnemies = static_cast<int>(floor(((double)curStage - 0) * INCREASE_RATE_OF_ENEMY_PAR_STAGE + DEFAULT_NUM_OF_ENEMY));
	if (numOfEnemies > MAX_NUM_OF_ENEMY) {
		numOfEnemies = MAX_NUM_OF_ENEMY;
	}
	//敵が生成される位置をnumOfEnemiesの文だけ設定
	double prob;
	while ((int)EPlace.size() < numOfEnemies) {
		prob = (double)randProbability(mt);
		int placement = beginToSpawn;
		while (prob > spawnPosibility[placement]) {
			if (placement == SWIDTH - 1) {
				break;
			}
			placement++;
			prob -= spawnPosibility[placement];
		}
		//同じ場所には出現しない
		if (EPlace[placement]) {
			continue;
		}
		EPlace[placement] = true;
		double sumOfDecreasePosibility = 0.0;
		//デバッグ用(生成確率の合計が100か確認するため)
		//double sumOfPosibility = 0.0;
		int startDecrease = max(placement - DECREASE_POSIBILITY_RANGE, beginToSpawn), endDecrease = min(placement + DECREASE_POSIBILITY_RANGE, SWIDTH - 1);
		int decreaseLength = endDecrease - startDecrease + 1;
		for (int i = startDecrease; i <= endDecrease; i++) {
			double before = spawnPosibility[i];
			spawnPosibility[i] *= DECREACE_RATE_OF_POSIBILITY;
			sumOfDecreasePosibility += before - spawnPosibility[i];
			//sumOfPosibility += spawnPosibility[i];
		}
		//確率を減らした分他の部分の出現確率を均等に増加させる
		for (int i = beginToSpawn; i < SWIDTH; i++) {
			if (i < startDecrease || endDecrease < i) {
				spawnPosibility[i] += (sumOfDecreasePosibility / ((double)spawnLength - decreaseLength));
				//sumOfPosibility += spawnPosibility[i];
			}
		}
	}
	for (int i = 0; i < SWIDTH; i++) {
		if (EPlace[i]) {
			CreateOneEnemy(i, randProbability(mt));
		}
	}
}
...
void Camera::CreateOneEnemy(int placement, int rand) {
	...
	if (rand % 2 == 0 && placement % ONESCAF != 0) {
		//半分の確率で下を向いた状態で出現
		oneEnemy->ReverseWorld();
	}
...
void Camera::RemoveAllEnemies() {
	while ((int)EntityList[1].size() > 0) {
		...
		for (int i = 0; i < NUM_OF_PARTICLES; i++) {
			del->GetParticle(i)->SetState(State::Dead);
		}
...
void Camera::SetUpAnimation(int x, int y, int& start, int& end) {
	if (GetMap(x, y) == BLOCK_BOUND) {
		...
		if (GetBound(x / ONESCAF) >= GetBound(x / ONESCAF - 1)) {
			if (y == GetBound(x / ONESCAF)) {
				start = BL_CORNER;
				end = BL_CORNER;
			}
			else if (y == GetBound(x / ONESCAF - 1)){
				start = WR_CORNER;
				end = WR_CORNER;
			}
			else {
...
void Camera::SetDisplayInfo() {
	...
	double maxPlacementCorrector = afterSize / 2.0 + 2.0;//2.0はスラッシュのための空間
	BulletSlash->SetPosition(Dir_Vector(ONEBOX * 2.0 + ONEBOX * maxPlacementCorrector, Game::getHeight() - ONEBOX * 2.5));
	maxPlacementCorrector += 1.5;//最大弾数を表示するまでのスラッシュとの距離
	for (int i = 0; i < afterSize; i++) {
		if (i >= currentSize) {
			Actor* Display = new Actor(GetGame());
			Max_Bullet_Actor.emplace_back(Display);
...
void Camera::CreateGameOver() {
	...
	//ダメージを受けた回数が10回になるにつれて倍率が2->1倍にその後は100回になるにつれて倍率が1->0.6倍になる
	if (damageTime < 10) {
	...
	//反転回数が20回になるにつれて倍率が1.5->1倍にその後は100回になるにつれて倍率が1->0.7倍になる
	if (revTime < 20) {
	...
	//ステージクリア時間が100秒に近づくにつれて倍率が1.5->1倍にその後は1倍のまま
	if (stageSumTime < 100) {
...
int Camera::GetStageScore() {
	//1ステージクリアごとに基礎スコアが200アップする(基本は100)
	double score = BASIC_SCORE + (double)INCREASE_SCORE * ((double)curStage - 1);
	int revTime = mPlayer->GetNoReverse();
	int damageTime = mPlayer->getNoDamage(); 
	int sumOfKills = mPlayer->GetKill(); 
